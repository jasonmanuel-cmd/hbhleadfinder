...
export function buildDossier(filing, names, rows, truncated) {
  ...
  // A default/trustee's sale proves at least one loan is still open, whatever the reconveyance count says.
  const isForeclosure = ['notice_of_default', 'notice_of_trustee_sale'].includes(filing.signal);
  const openLoans = Math.max(isForeclosure ? 1 : 0, dots.length - recons.length);
  ...
  // Only the last 15 years count — a 1980s default says nothing about today.
  const yearBefore = new Date(new Date(fdate).getTime() - 365 * 86400000).toISOString().slice(0, 10);
  const fifteenBefore = new Date(new Date(fdate).getTime() - 15 * 365.25 * 86400000).toISOString().slice(0, 10);
  const priorDefaults = nods.filter((r) => r.document_number !== filing.document_number
    && r.recorded_date < yearBefore && r.recorded_date >= fifteenBefore).length;
  const foreclosed = trusteeDeeds.some((r) => r.recorded_date >= fdate);
  const transferredOut = sells.some(after);

  let equityHint = 'unknown';
  if (!isForeclosure && openLoans === 0 && dots.length > 0) equityHint = 'high';   // every recorded loan was paid off
  ...
  // Common names pull in other people's documents — don't let them drive equity or priority.
  if (truncated) equityHint = 'unverified';
  ...
  const priorCures = cancels.filter((r) => r.recorded_date < fdate && r.recorded_date >= fifteenBefore).length;
  ...
  if (truncated) summary.push('Common name — history mixes other people; verify by hand');
