...
export function buildDossier(filing, names, rows, truncated) {
  ...
  const priorDefaults = nods.filter((r) => r.document_number !== filing.document_number && r.recorded_date < fdate).length;
  ...
  if (priorDefaults) summary.push(`${priorDefaults} earlier default${priorDefaults > 1 ? 's' : ''}${cancels.length ? ` (${cancels.length} cured)` : ''}`);
  ...
  return {
    ...
    signals: {
      ...
      prior_defaults: priorDefaults, cured_defaults: cancels.length, open_liens: openLiens, solar, deceased,
