...
async function kernFromAddress({ address, city, zip }) {
  ...
  for (const c of cands.filter((c) => c.score >= top - 3).slice(0, 4)) {
    ...
    hits.push({ apn: apns[0], address: tidyStreet(a.StAddr || c.address.split(',')[0]),
      city: titleCase(a.City || ''), zip: digits(a.ZIP).slice(0, 5) || null, score: c.score });
  }
  ...
  // fill ZIP/city from whichever candidate had them
  const h = distinct[0];
  return { ...h, city: h.city || hits.find((x) => x.city)?.city || null, zip: h.zip || hits.find((x) => x.zip)?.zip || null };
}
