...
export async function fixPropertyAddress(propertyId, { address, city, zip }) {
  ...
  const res = await sql`
    update properties set address_line_1 = ${address}, city = coalesce(${city}, city), zip = coalesce(${zip}, zip),
           address_key = normalize_address(${address}), updated_at = now()
     where id = ${propertyId} and not real_address(address_line_1)
    returning id`;
  return res.length > 0;
}
